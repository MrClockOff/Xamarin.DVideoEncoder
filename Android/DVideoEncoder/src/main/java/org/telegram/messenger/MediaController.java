...
public class MediaController {
    ...
    private ArrayList<MediaControllerProgressListener> _progressListeners = new ArrayList<>();
    private ArrayList<MediaControllerLogListener> _logListeners = new ArrayList<>();

    public MediaController(Context c) { context = c; }

    public void setProgressListener(MediaControllerProgressListener listener) {
    ...
    public void removeProgressListener(MediaControllerProgressListener listener) {
        if(!_progressListeners.contains(listener)) {
            return;
        }
    ...
    public void setLogListener(MediaControllerLogListener listener) {
    ...
    public void removeLogListener(MediaControllerLogListener listener) {
        if(!_logListeners.contains(listener)) {
            return;
        }
    ...
    public void scheduleVideoConvert(VideoEditedInfo videoEditedInfo) {
        videoConvertQueue.add(videoEditedInfo);
    ...
    public void cancelVideoConvert(VideoEditedInfo videoEditedInfo) {
        if (videoEditedInfo == null) {
            ...
            if (!videoConvertQueue.isEmpty()) {
                if (videoConvertQueue.get(0) == videoEditedInfo) {
                    synchronized (videoConvertSync) {
                        cancelCurrentVideoConversion = true;
                    }
                }
                videoConvertQueue.remove(videoEditedInfo);
            }
        }
    }
    ...
    private void startVideoConvertFromQueue() {
        if (!videoConvertQueue.isEmpty()) {
            ...
            VideoEditedInfo videoEditedInfo = videoConvertQueue.get(0);
            VideoConvertRunnable.runConversion(this, videoEditedInfo);
        }
    }
    ...
    private void didWriteData(final VideoEditedInfo videoEditedInfo, final File file, final boolean last, final boolean error) {
    ...
    @TargetApi(16)
    private long readAndWriteTrack(final VideoEditedInfo videoEditedInfo, MediaExtractor extractor, MP4Builder mediaMuxer, MediaCodec.BufferInfo info, long start, long end, File file, boolean isAudio) throws Exception {
    ...
    private static class VideoConvertRunnable implements Runnable {
        ...
        private VideoConvertRunnable(MediaController controller, VideoEditedInfo video) {
            videoEditedInfo = video;
            mediaController = controller;
        }

        @Override
        public void run() {
            mediaController.convertVideo(videoEditedInfo);
        }

        public static void runConversion(final MediaController ctr, final VideoEditedInfo obj) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    try {
                        VideoConvertRunnable wrapper = new VideoConvertRunnable(ctr, obj);
                        Thread th = new Thread(wrapper, "VideoConvertRunnable");
                        th.start();
                        th.join();
                    } catch (Exception e) {
                        ctr.FileLog("tmessages", e);
                    }
                }
            }).start();
        }
    ...
    @TargetApi(16)
    private boolean convertVideo(final VideoEditedInfo videoEditedInfo) {
        ...
        int rotateRender = 0;
        File cacheFile = new File(videoEditedInfo.cacheFile);
        ...
        SharedPreferences preferences = context.getApplicationContext().getSharedPreferences("videoconvert", Activity.MODE_PRIVATE);
        ...
        if (!inputFile.canRead() || !isPreviousOk) {
            didWriteData(videoEditedInfo, cacheFile, true, true);
        ...
        if (resultWidth != 0 && resultHeight != 0) {
            ...
            try {
                ...
                if (resultWidth != originalWidth || resultHeight != originalHeight || rotateRender != 0) {
                    ...
                    if (videoIndex >= 0) {
                        ...
                        try {
                            ...
                            if (Build.VERSION.SDK_INT < 18) {
                                ...
                                FileLog("tmessages", "codec = " + codecInfo.getName() + " manufacturer = " + manufacturer + "device = " + Build.MODEL);
                            } else {
                                colorFormat = MediaCodecInfo.CodecCapabilities.COLOR_FormatSurface;
                            }
                            FileLog("tmessages", "colorFormat = " + colorFormat);
                                ...
                                while (decoderOutputAvailable || encoderOutputAvailable) {
                                        ...
                                        if (info.size > 1) {
                                            if ((info.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) == 0) {
                                                if (mediaMuxer.writeSampleData(videoTrackIndex, encodedData, info, false)) {
                                                    didWriteData(videoEditedInfo, cacheFile, false, false);
                                                }
                                    ...
                                    if (!decoderDone) {
                                        ...
                                        if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                                            ...
                                            FileLog("tmessages", "newFormat = " + newFormat);
                                            ...
                                            if (startTime > 0 && videoTime == -1) {
                                                if (info.presentationTimeUs < startTime) {
                                                    doRender = false;
                                                    FileLog("tmessages", "drop frame startTime = " + startTime + " present time = " + info.presentationTimeUs);
                                                } else {
                                                ...
                                                if (!errorWait) {
                                                    if (Build.VERSION.SDK_INT >= 18) {
                                                        ...
                                                        if (inputBufIndex >= 0) {
                                                            ...
                                                            FileLog("tmessages", "input buffer not available");
                                                        }
                                                    }
                                                }
                                            }
                                            if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                                                decoderOutputAvailable = false;
                                                FileLog("tmessages", "decoder stream end");
                                                if (Build.VERSION.SDK_INT >= 18) {
                    ...
                    long videoTime = readAndWriteTrack(videoEditedInfo, extractor, mediaMuxer, info, startTime, endTime, cacheFile, false);
                    if (videoTime != -1) {
                        videoStartTime = videoTime;
                    }
                }
                if (!error && bitrate != -1) {
                    readAndWriteTrack(videoEditedInfo, extractor, mediaMuxer, info, videoStartTime, endTime, cacheFile, true);
                }
            } catch (Exception e) {
                error = true;
                FileLog("tmessages", e);
                ...
                FileLog("tmessages", "time = " + (System.currentTimeMillis() - time));
            }
        } else {
            preferences.edit().putBoolean("isPreviousOk", true).commit();
            didWriteData(videoEditedInfo, cacheFile, true, true);
        ...
        preferences.edit().putBoolean("isPreviousOk", true).commit();
        didWriteData(videoEditedInfo, cacheFile, true, error);
        return true;
    }
    ...
    private void FileLog(String tag, Exception e) {
        FileLog(tag, e.getMessage());
    }
